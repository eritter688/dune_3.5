...
#define FUNC_ARG_TYPE_ERROR(F, N)      (object_name() +"::" +(F) +"(): Bad type to argument " +(N) +".\n")
#endif
...
public varargs mapping parse_time(mixed date_text, string date_fmt) {
  ...
  string *fmt_bits = regexplode(date_fmt || CTIME_FORMAT, implode(m_indices(patterns), "|"));
